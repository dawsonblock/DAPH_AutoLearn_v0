...
def test_total_groups_is_60():
    assert N_GROUPS_TOTAL == 60
...
def test_split_proportions():
    a = group_assignments()
    assert len(a["train"]) == 30
    assert len(a["development"]) == 12
    assert len(a["calibration"]) == 9
    assert len(a["final"]) == 9
...
def test_generate_all_splits():
    ...
    assert counts["calibration"] == 9
    assert counts["final"] == 9
